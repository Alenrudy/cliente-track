...
function sendClientSmartQRWA() {
  ...
  let method = '';
  if (currentClientQRType === 'yape') method = `Yape al ${state.config?.yapeNum || ''}`;
  else if (currentClientQRType === 'plin') method = `Plin al ${state.config?.plinNum || ''}`;
  else if (currentClientQRType === 'empresa') method = `Pago Empresa: ${state.config?.businessInfo || ''}`;
  
  const msg = `Hola ${client.name}, para proceder con tu pedido ${client.product}, puedes pagar S/ ${parseFloat(client.cobrar).toFixed(2)} vía ${method}. ¡Gracias!`;
